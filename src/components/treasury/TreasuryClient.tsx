...
export function TreasuryClient({ userId, isAdmin, initialTransactions, initialBalance }: Props) {
  ...
  const typeConfig = {
    income: { label: 'Entrée', icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', sign: '+' },
    expense: { label: 'Dépense', icon: TrendingDown, color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/20', sign: '-' },
    adjustment: { label: 'Ajustement', icon: ArrowLeftRight, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20', sign: '=' },
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center">
        ...
        <p className="text-zinc-600 text-xs mt-2">{transactions.length} transaction{transactions.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <button
          ...
          className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-sm font-medium transition-colors ${
            mode === 'income' ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:border-emerald-500/50 hover:text-emerald-400'
          }`}
        >
          <Plus size={18} />
          Entrée
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-sm font-medium transition-colors ${
            mode === 'expense' ? 'bg-red-500 border-red-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:border-red-500/50 hover:text-red-400'
          }`}
        >
          <Minus size={18} />
          Dépense
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-sm font-medium transition-colors ${
            mode === 'adjustment' ? 'bg-blue-500 border-blue-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:border-blue-500/50 hover:text-blue-400'
          }`}
        >
                ...
                <li key={t.id} className="group flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-zinc-500 mt-0.5">
                      {(t.profiles as any)?.name} · {format(new Date(t.created_at), 'd MMM yyyy, HH:mm', { locale: fr })}
                    </div>
                  </div>
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-red-400 transition-all disabled:opacity-50 ml-1"
                    >
